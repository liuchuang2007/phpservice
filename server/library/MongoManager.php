...
/**
 *@name: MongoManager
 *@description: manipulate mongodb
 *@author: liuchuang
 *@Date:2013-08-14
 */
class MongoManager {
    ...
    public function __construct(array $conf) {
        if(!class_exists('Mongo')):
            show_error("201", 500);//The MongoDB PECL extension has not been installed or enabled
        endif;
        $this->connection_string($conf);
        $this->connect();
    }
    ...
    public function switch_db($database = '') {
        if(empty($database)):
            show_error("203", 500);//To switch MongoDB databases, a new database name must be specified
        endif;
        ...
        try {
            ...
            show_error("205", 500);//Unable to switch Mongo Databases
        }
    }
     ...
     */
     public function get($collection = "") {
        if(empty($collection))
            show_error("207", 500);//In order to retreive documents from MongoDB, a collection name must be passed
        $results = array();
     ...
     */
     public function count($collection = "") {
        if(empty($collection))
            show_error("209", 500);//In order to retreive a count of documents from MongoDB, a collection name must be passed
     ...
     */
     public function insert($collection = "", $data = array()) {
        if(empty($collection))
            show_error("211", 500); //No Mongo collection selected to insert into
        if(count($data) == 0 || !is_array($data))
            show_error("213", 500); //Nothing to insert into Mongo collection or insert is not an array
        
        try {
            $result = $this->db->{$collection}->insert($data, array('safe' => TRUE));
			return $result['ok'];
        } catch(MongoCursorException $e) {
            show_error("215", 500);//Insert of data into MongoDB failed
        }
     ...
     */
     public function update($collection = "", $data = array()) {
        if(empty($collection))
            show_error("217", 500);//No Mongo collection selected to update
        if(count($data) == 0 || !is_array($data))
            show_error("219", 500);//Nothing to update in Mongo collection or update is not an array
     ...
     */
     public function update_all($collection = "", $data = array()) {
        ...
        try {
            ...
            show_error("221", 500);//Update of data into MongoDB failed
        }
     }
     ...
     */
     public function delete($collection = "") {
        if(empty($collection))
            show_error("223", 500);//No Mongo collection selected to delete from
        
        try {
            $this->db->{$collection}->remove($this->wheres, array('safe' => TRUE, 'justOne' => TRUE));
            return(TRUE);
        } catch(MongoCursorException $e) {
            show_error("225", 500);//Delete of data into MongoDB failed
        }
    ...
    public function add_index($collection = "", $keys = array(), $options = array()) {
        if(empty($collection))
            show_error("227", 500);//No Mongo collection specified to add index to
        if(empty($keys) || !is_array($keys))
            show_error("229", 500);//Index could not be created to MongoDB Collection because no keys were specified
        ...
        if($this->db->{$collection}->ensureIndex($keys, $options) == TRUE):
            ...
            show_error("231", 500);//An error occured when trying to add an index to MongoDB Collection
        endif;
    }
    ...
    public function remove_index($collection = "", $keys = array()) {
        if(empty($collection))
            show_error("233", 500);//No Mongo collection specified to remove all indexes from
        if(empty($keys) || !is_array($keys))
            show_error("235", 500);//Index could not be removed from MongoDB Collection because no keys were specified
        if($this->db->{$collection}->deleteIndex($keys, $options) == TRUE):
            $this->clear();
            return($this);
        else:
            show_error("237", 500);//An error occured when trying to remove an index from MongoDB Collection
        endif;
    }
    ...
    private function connect() {
        ...
        try {
            ...
            show_error("239", 500);//Unable to connect to MongoDB:
        }
    }
     ...
     */
    private function connection_string($conf) {
        $this->host = trim($conf['host']);
        $this->port = trim($conf['port']);
        $this->user = trim($conf['user']);
        $this->pass = trim($conf['pass']);
        $this->dbname = trim($conf['db']);
        $this->persist = trim($conf['persist']);
        $this->persist_key = trim($conf['persist_key']);
        
        $connection_string = "mongodb://";
        
        if(empty($this->host)):
            show_error("241", 500);//The Host must be set to connect to MongoDB
        endif;
        
        if(empty($this->dbname)):
            show_error("243", 500);//The Database must be set to connect to MongoDB
        endif;
